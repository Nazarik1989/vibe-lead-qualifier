...
Компактное тестовое задание для вакансии AI-разработчика: FastAPI-сервис принимает сообщения из VibeMarketolog Agent API, собирает структурированный бриф лида и, когда данных достаточно, возвращает разрешённое CRM-действие `crm.deal.add` для Bitrix24.
...
## Публичный HTTPS webhook

Официальная платформа принимает только публичный HTTPS URL и отклоняет loopback, приватные и зарезервированные адреса. Перед запуском туннеля установите `ENABLE_DEMO_ENDPOINTS=false` в `.env`, перезапустите приложение и убедитесь, что `/demo/*` возвращает `404`. После этого оставьте приложение на `127.0.0.1:8000` и поднимите временный туннель отдельным процессом.
...
Добавьте к выданному HTTPS-домену путь `/webhooks/vibe`, например `https://random.example-tunnel.com/webhooks/vibe`. Туннель открывает локальный сервис интернету; после проверки остановите его.
...
```powershell
$CallbackUrl = "https://random.example-tunnel.com/webhooks/vibe"
...
Demo-маршруты по умолчанию отключены. Если их разрешать вне локальной среды, потребуются авторизация и rate limiting; для production также нужны политика хранения персональных данных, шифрование/секрет-хранилище, наблюдаемость и распределённое хранилище локальной идемпотентности.
...
- при явном включении локальные demo-endpoints не аутентифицированы и не имеют rate limiting;
...
4. Rate limiting, OpenTelemetry, метрики задержки/дублей и алерты.
